...
@pytest.mark.parametrize(
    ("label", "kwargs", "expect_declined"),
    [
        ("d64_dense_f16", {}, False),
        ("d128_dense_f16", {"d": 128}, True),
        ("d192_dense_f16", {"d": 192}, True),
    ],
)
def test_probe_declines_head_dims_above_the_box(label, kwargs, expect_declined):
    ...
    caps = _spec().capabilities
    # The probe compares device cc first; pin it to Ada so the head-dim rule is
    ...
    assert declined is expect_declined, f"{label}: mismatch() said {reason!r}"
