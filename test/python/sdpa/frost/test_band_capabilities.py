...
def test_declaring_flags_and_band_together_is_explicit(kind):
    ...
    # A contradiction is refused, naming both spellings.
...
def test_keyword_construction_and_dataclasses_replace(engines_module):
    ...
    # Changing a mask FLAG of a declared row contradicts the copied claim and
    # must raise rather than silently re-deriving.
    with pytest.raises(ValueError):
        dataclasses.replace(flagged, causal=False)
