...
def _sm80_device_family(device_cc) -> str:
    ...
    raise ValueError(
        f"the SM80-family SDPA kernels are validated on cc {known} only; found SM{device_cc[0]}{device_cc[1]}"
    )
...
class SdpaFwdDslSm80(SdpaFwdDsl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._declared_device_cc = tuple(device_cc) if device_cc is not None else None
    ...
    def check_support(self) -> bool:
        ...
        if self._declared_device_cc is not None:
            self._value_error_if(
                ...
                f"SdpaFwdDslSm80 was built for cc {sorted(self._declared_device_cc)}; "
                f"found SM{major}{minor} on {device}",
            )
        else:
            self._value_error_if(
                self._device_cc != (8, 0),
