...
flags.  Declaring both spellings is allowed only while they agree, and
disagreeing raises at construction (see ``Capabilities.__post_init__`` in
``fwd/engines.py`` / ``bwd/engines.py``) — never silently resolved by field
order.
...
asking the three axis questions costs ~1.6 us per probe that reaches the band
block, against ~0.2 us for the four raw boolean reads it replaces (see
``evidence/601/tools/band_support_cost.py``); the probe builds it only after the
arch / dtype / shape declines, and ``BandSupport`` itself is resolved once at
import.  That is a representation cost, not a speed win -- the value here is
that a RESTRICTED claim is expressible at all.
"""
...
REASON_RIGHT = {
    RIGHT_UNBOUNDED: feature_reason("unmasked band"),
    RIGHT_CAUSAL: feature_reason(LABEL_RIGHT_CAUSAL),
    RIGHT_FINITE: feature_reason(LABEL_RIGHT_FINITE),
}
REASON_LEFT_WINDOW = feature_reason(LABEL_LEFT_WINDOW)
REASON_ANCHOR_WITHOUT_DIAGONAL = "bottom-right alignment requires a causal upper bound (plain or right-widened)"


def anchor_reason(noun: str = "kernel") -> str:
    """The bottom-right-anchor decline sentence.
    ...
    instead of silently rewording one pass during the migration.
    """
    return f"graph uses bottom-right causal, which this {noun} does not support"
